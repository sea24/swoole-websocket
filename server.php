...
$server->on('message', function (swoole_websocket_server $_server, $frame) use ($server) {
    if ($frame->fd == 1) {
        $_server->push(2, $frame->data);
    }
    if ($frame->fd == 2) {
